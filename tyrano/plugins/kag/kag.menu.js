    ...
    displaySave:function(cb){
          ...
          j_save.find(".save_display_area").each(function(){
            $(this).click(function(e){
              ...
              setTimeout(function(){
                ...
                $('.mySave-popup').animate({"opacity": 1}, 200, function(){
                  setTimeout(function(){
                    $('.mySave-popup').animate({"opacity": "0"}, 200, function(){
                      layer_menu.hide();
                      layer_menu.empty();
                    });
                  }, 300);
      ...
      doSave:function(num,cb){
        ...
        if(this.snap==null)this.snapSave(this.kag.stat.current_save_str,function(){
          ...
          // add -----------------------------------------
          data.scene_title=that.kag.stat.f.sceneTitle;
          // add -----------------------------------------
          ...
          if(typeof cb=="function")cb()});
            ...
            setQuickSave:function(){
              ...
              loadAutoSave:function(){
                ...
                if(data)data=JSON.parse(data);else return false;
                     ...
                     if(that.kag.stat.save_img!=""){
                      ...
                      html2canvas(tmp_base.get(0),{
                        onrendered:function(canvas){
                          ...
                          var img_code=that.createImgCode(canvas);
                          completeImage(img_code)
                        },height:that.kag.config.scHeight,width:that.kag.config.scWidth});
                      tmp_base.hide();tmp_base.css("left",tmp_left);
                ...
                this.kag.html("load",{array_save:array,"novel":$.novel},function(html_str){
                  ...
                  j_save.find(".save_display_area").each(function(){
                    $(this).click(function(e){var num=$(this).attr("data-num");
                      ...
                      layer_menu.empty();
                      if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()})});
                  j_save.find(".button_smart").hide();if($.userenv()!="pc"){j_save.find(".button_smart").show();
                  ...
                  j_save.find(".button_arrow_down").click(function(){var now=j_save.find(".area_save_list").scrollTop();
                    var pos=now+160;
                    j_save.find(".area_save_list").animate({scrollTop:pos},{queue:false})})}
                  var layer_menu=that.kag.layer.getMenuLayer();that.setMenu(j_save,cb)})},
              loadGame:function(num){var array_save=this.getSaveData();var array=array_save.data;if(array[num].save_date=="")return;var auto_next="no";if(array[num].stat.load_auto_next==true)auto_next="yes";this.loadGameData($.extend(true,{},array[num]),{"auto_next":auto_next})},

              loadGameData:function(data,options){var auto_next="no";if(typeof options=="undefined")options=
...
setMenu:function(j_obj,cb){
  ...
  j_obj.find(".menu_close").click(function(e){
    layer_menu.fadeOut(100,function(){
      layer_menu.empty();
      if(typeof cb=="function")cb()});
    if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()
  });
  ...
  $.preloadImgCallback(layer_menu,function(){
    j_obj.show();
    layer_menu.find(".block_menu").hide()
},that)},
...
getSaveData:function(){
  ...
  for(var i=0;i<save_slot_num;i++){
    ...
    json.img_data="";
    json.stat={};
    tmp_array.push(json)}root.data=tmp_array;
    return root}},
...
displayLog:function(){
  ...
  this.kag.html("backlog",{"novel":$.novel},function(html_str){
    var j_menu=$(html_str);
    var layer_menu=that.kag.layer.getMenuLayer();
    layer_menu.empty();layer_menu.append(j_menu);
    layer_menu.find(".menu_close").click(function(){
      layer_menu.fadeOut(100,function(){layer_menu.empty()});
      if(that.kag.stat.visible_menu_button==true)$(".button_menu").show()});
    layer_menu.find(".button_smart").hide();
if($.userenv()!="pc"){layer_menu.find(".button_smart").show();
...
layer_menu.find(".button_arrow_down").click(function(){
  var now=layer_menu.find(".log_body").scrollTop();
  var pos=now+60;layer_menu.find(".log_body").animate({scrollTop:pos},{queue:false})})}
var log_str="";var array_log=that.kag.variable.tf.system.backlog;
...
$.preloadImgCallback(layer_menu,function(){layer_menu.show();
  layer_menu.find(".log_body").scrollTop(9999999999)},that);
$(".button_menu").hide()})},

screenFull:function(){if($.isNWJS()==true){var gui=require("nw.gui");var win=gui.Window.get();if(win.isFullscreen)win.leaveFullscreen();else win.enterFullscreen()}else{var isFullScreen=
...
else elem.webkitRequestFullscreen();else if(elem.mozRequestFullScreen)if(isFullScreen)document.mozCancelFullScreen();else elem.mozRequestFullScreen();else if(elem.msRequestFullscreen)if(isFullScreen)document.msExitFullscreen();else elem.msRequestFullscreen()}},
